...
def print_stream_density_report(num_pipelines, stream_fps_dict,
    ...
    for name in stream_fps_dict:
        ...
        ratio = (measured / pass_mark) if pass_mark > 0 else float('inf')
        if binding_ratio is None or ratio < binding_ratio:
            binding_ratio = ratio
            binding_name = name
    ...
    print("-" * 47)
    camera_stream_count = len(stream_fps_dict)
    ...
    print("")
    print(
        f"{'camera':<8}{'workload':<50}{'target':>8}{'pass mark':>11}"
    ...
    for name in sorted(
        ...
        print(
            f"{camera:<8}{workload:<50}{target:>8.2f}{pass_mark:>11.2f}"
            f"{measured:>10.2f}{result:>8}")
    print("")
    print(
        f"Verdict: {'pass' if overall_pass else 'fail'} - "
        f"{camera_stream_count} streams sustained")
    if binding_name is not None:
        ...
        if uniform_targets:
            stream_label = "slowest camera"
            targets_clause = f"Every camera targets {bt:.2f} fps with a pass mark of {bp:.2f} fps."
        else:
            stream_label = "binding camera"
            targets_clause = "Targets differ per camera."
        print(
            f"The run was evaluated over {consecutive_pass_windows} consecutive "
            f"measurement windows of {measurement_window_seconds} seconds each. "
            f"{targets_clause} The {stream_label} was {binding_camera}, which "
            f"measured {bm:.2f} fps against its {bp:.2f} fps mark and "
            f"{'passed' if bm >= bp else 'failed'}. "
            f"{camera_stream_count} streams sustained.")
...
def run_pipeline_iterations( 
    ...
    clean_up_pipeline_logs(results_dir)
    target_fps_label = (
        f"TARGET_FPS set for {target_fps}" if explicit_target_fps
        else f"Target FPS per-camera configuration is considered")
    print(
